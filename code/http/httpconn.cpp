...
/**
 * iov[0]是Buffer缓冲区内的数据
 * iov[1]是mmfile，内存映射的数据
 * 这个函数实现的是将这些数据发送给客户端，通过fd_
 */
ssize_t HttpConn::write(int* saveErrno) {
  ...
  do {
    ...
    // 下面的代码就做了一件事，把已经读过的数据删除，调整iov——base的指向和iov的大小
...
bool HttpConn::process() {
  ...
  response_.MakeResponse(writeBuff_);  // 准备响应信息
  /* 响应头 */
  ...
  // 到这一步，只是将writeBuffer和mmfile中的数据进一步封装到iov里面，还未传输数据，数据传输函数在HttpConn::write
